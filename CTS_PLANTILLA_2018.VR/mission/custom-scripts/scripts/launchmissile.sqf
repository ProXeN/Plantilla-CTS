//nul=[laserTarget player, [getpos Barracuda select 0, getpos Barracuda select 1,(getpos Barracuda select 2) + 200],RHS_9M79B",200] execvm "mission\custom-scripts\scripts\launchmissile.sqf"
...
if (isNull _primarytarget) exitWith {hintSilent "¡Objetivo no Encontrado! Necesitas un designador laser."};

if (isNil "countMissiles") then {countMissiles = 0;} else {countMissiles = countMissiles + 1;};
    publicVariable "countMissiles";
if (countMissiles > 2) exitWith {hintSilent "Solo dispones de dos lanzamientos de misiles."};
