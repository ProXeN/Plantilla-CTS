#define TEXTURE_BACKGROUND	"\CTS_assets\timeline\background_ca.paa"
#define TEXTURE_TIMELINE	"\CTS_assets\timeline\timeline_ca.paa"
...
private _campaign = getText (missionconfigfile >> "CfgCampaign" >> "campaignTitle");
_ctrlTitle ctrlsetstructuredtext parsetext format ["<t shadow='2' align='center' font ='PuristaBold' size='2'>%1</t>", _campaign];
...
if (_centered && _fadeIn > 0) then {playsound ["Timeline_FadeIn",true];};
...
private _commitTime = COMMIT_TIME_MISSION;
{
	...
	private _done = getnumber (_x >> "isDone") > 0;
	private _size = if (_done) then {ICON_SIZE_PLAYED} else {ICON_SIZE_UNKNOWN};
	...
	if (_done || _mission == _missionCurrent) then {
	...
	if (_mission == _missionCurrent) then {
		...
		_commitTime = COMMIT_TIME_MISSION;
...
} foreach configproperties [missionconfigfile >> "cfgCampaign" >> "Missions"];
_ctrGroup ctrlsetfade 0;
_ctrGroup ctrlcommit 0;

//--- Delay current icon animation
_time = time + DELAY_ICON;
waituntil {!_centered || {time > _time}};

//--- Fade in current icon
waituntil {!_centered || {ctrlcommitted _ctrlLine}};
...
if (_centered) then {playsound ["Timeline_IconFadeIn",true];};

//--- Enlarge current icon
waituntil {ctrlcommitted _ctrlIconCurrent};
...
//--- Delay
waituntil {!_centered || {ctrlcommitted _ctrlLabel1}};
_time = time + DELAY_DATE;

//--- Show second part of the date
waituntil {time > _time|| !_centered};
_ctrlLabel2 ctrlsetfade 0;
_ctrlLabel2 ctrlcommit (_commitTime);
if (_centered) then {playsound ["Timeline_DateFadeIn",true];};

//--- Wait
waituntil {!_centered || {ctrlcommitted _ctrlIconCurrent}};
...
_ctrlTitle ctrlsetFade 1;
_ctrlTitle ctrlcommit _fadeOut;
_ctrlLabel1 ctrlsetfade 1;
...
if (_centered && _fadeOut > 0) then {playsound ["Timeline_FadeOut",true];};

waituntil {ctrlcommitted _ctrGroup};
call _fnc_terminate;
