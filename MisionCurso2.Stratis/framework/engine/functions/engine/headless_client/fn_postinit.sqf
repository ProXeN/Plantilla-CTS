_mission_enable_hc = "HCEnabled" call BIS_fnc_getParamValue;
mission_AI_controller = if (_mission_enable_hc > 0) then { !isServer && !hasInterface } else { isServer };
if !(isMultiplayer) then {mission_AI_controller = true};
