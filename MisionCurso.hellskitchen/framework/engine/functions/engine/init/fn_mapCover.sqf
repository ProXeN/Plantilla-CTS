...
//[markerName,specLogic,hatchAreaFactor,aoProtection,desertMapColors] call f_fnc_missionao;
//Example: ["aom",specPen,1000,true,false] call f_fnc_missionao;
if !(hasInterface) exitWith {};

if (getMarkerColor "AO" == "") then {
_marker = createMarker ["AO", [getNumber (configfile >> "CfgWorlds" >> worldName >> "mapSize") / 2, getNumber (configfile >> "CfgWorlds" >> worldName >> "mapSize") / 2]];
_marker setMarkerShape "RECTANGLE";
_marker setMarkerSize [getNumber (configfile >> "CfgWorlds" >> worldName >> "mapSize") / 2, getNumber (configfile >> "CfgWorlds" >> worldName >> "mapSize") / 2];
