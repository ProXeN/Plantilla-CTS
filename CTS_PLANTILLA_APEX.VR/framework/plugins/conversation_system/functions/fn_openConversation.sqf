...
_picture = _first getVariable ["IP_Avatar", "framework\plugins\conversation_system\img\defaultAvatar.jpg"];
ctrlSetText [1200, _picture];
_rank = if ((side _first) != civilian) then {if (!isNil "BRM_conversation_system_fnc_getRank") then {(([_first, false] call BRM_conversation_system_fnc_getRank) + "<br/>")} else {(rank _first) + "<br/>"}} else {""};
...
_picture = _second getVariable ["IP_Avatar", "framework\plugins\conversation_system\img\defaultAvatar.jpg"];
ctrlSetText [1201, _picture];
_rank = if ((side _second) != civilian) then {if (!isNil "BRM_conversation_system_fnc_getRank") then {(([_second, false] call BRM_conversation_system_fnc_getRank) + "<br/>")} else {(rank _second) + "<br/>"}} else {""};
...
if (count _sentences == 0) exitWith {
	call BRM_conversation_system_fnc_closeConversation;
...
if (_exit == 1) then {call BRM_conversation_system_fnc_closeConversation};
