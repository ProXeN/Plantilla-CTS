...
_text = switch (_wichChat) do {
	case "SIDE": {
		...
		// player createDiarySubject ["ConvLog", "Conversation Log"];_sentence
		// player createDiaryRecord ["ConvLog", [_groupID, _sentence]];
		[_groupID,_sentence] call BRM_fnc_MessageLog;
	...
	case "GROUP": {
		...
		[_name,_sentence] call BRM_fnc_MessageLog;
	...
	case "VEHICLE": {
		...
		[_role,_sentence] call BRM_fnc_MessageLog;
	...
	case "GLOBAL": {
		...
		[(_factionName + " " + _groupID),_sentence] call BRM_fnc_MessageLog;
		_str = "<t size=" + _size + " color='#BDBDBD'>" + _factionName + " " + _groupID + ": " + _sentence + "</t>";
		_str
	};

	default {format ["BRM_conversation_system_fnc_simpleSentence:<br/>%1 not recognized!", _wichChat]};
};
...
if (_liveFeed && {(vehicle _speaker) == _speaker}) then {
	...
	_picture = _speaker getVariable ["IP_Avatar", "framework\plugins\conversation_system\img\defaultAvatar.jpg"];
