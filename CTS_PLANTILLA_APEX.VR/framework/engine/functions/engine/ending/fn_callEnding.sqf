...
if (!isRemoteExecuted && isMultiplayer || count _this == 1) then {
	...
	[_ending, _winningSides, _losingSides, _showStats, _title, _reason, _endNumber, _margin] remoteExec ["BRM_fnc_callEnding", 0];
} else {
	...
	if (hasInterface) then {
		...
		if (_showStats) then {
			...
			private _fnc_formatSide = {
				format ["<t color='%2'>%1</t>", [_x, "name"] call BRM_fnc_getSideInfo, [[_x, "color"] call BRM_fnc_getSideInfo] call BRM_fnc_colorToHex]
			};
			...
			private _lines = [
				...
				format [_reason,
					[_winningSides apply _fnc_formatSide] call BRM_fnc_verboseArray,
					[_losingSides apply _fnc_formatSide] call BRM_fnc_verboseArray,
					_margin
				],
				"",
			...
			if (mission_enable_side_c) then {
				_lines pushBack (format ["<t align='right'><t color='%1'>%2</t>: %3</t>",
					[side_c_color] call BRM_fnc_colorToHex,
					side_c_name,
					mission_dead_side_c
				]);
			};
